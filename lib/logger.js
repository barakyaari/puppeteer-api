...
const stringifySafe = require('json-stringify-safe'); // Prevents circular JSON issues
...
const logger = createLogger({
  ...
  format: format.combine(
    ...
    format.printf(({ timestamp, level, message, stack, ...meta }) => {
      // Handle circular references in meta safely
      let metaString = Object.keys(meta).length ? ` ${stringifySafe(meta, null, 2)}` : '';
      return stack
